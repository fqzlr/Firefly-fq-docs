...
相关文件：
- 配置文件：`musicConfig.ts`
- 音乐管理器：`MusicManager.astro`
- 播放器组件：`MusicPlayer.astro`
- 侧边栏小部件：`Music.astro`

## 基础配置

音乐播放器的配置文件位于 `src/config/musicConfig.ts`。

### 显示/隐藏播放器

播放器默认在两个位置显示：

1. **侧边栏小部件**：在 `sidebarConfig.ts` 中配置
...
音乐播放器在侧边栏的显示可以通过 `sidebarConfig.ts` 控制。
...
- 可视化组件目录：`music-visualizer/`
- 音乐页面路由：`/music/`
- 页面组件：`music/index.astro`
...
悬浮歌词的样式定义在 `floating-lyrics.css`，可以根据需要自定义样式。

## 相关文件链接

| 文件 | 说明 |
|------|------|
| `musicConfig.ts` | 音乐播放器配置文件 |
| `MusicManager.astro` | 音乐状态管理器（核心逻辑） |
| `MusicPlayer.astro` | 播放器 UI 组件 |
| `Music.astro` | 侧边栏音乐小部件 |
| `FloatingLyrics.astro` | 悬浮歌词组件 |
| `music-player.css` | 播放器样式 |
| `music-visualizer/` | 3D 可视化组件目录 |
