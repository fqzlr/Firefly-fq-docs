...
配置文件路径：`src/config/commentConfig.ts`

## 基础配置

| 配置项 | 类型 | 默认值 | 说明 |
|--------|------|--------|------|
| `type` | `"none" \| "twikoo" \| "waline" \| "giscus" \| "disqus" \| "artalk"` | `"waline"` | 评论系统类型，`none` 表示不启用评论系统 |
...
| 配置项 | 类型 | 默认值 | 说明 |
|--------|------|--------|------|
| `twikoo.envId` | `string` | `"https://twikoo.vercel.app"` | Twikoo 环境 ID 或服务地址 |
...
| 配置项 | 类型 | 默认值 | 说明 |
|--------|------|--------|------|
| `waline.serverURL` | `string` | `"https://co.tsh520.cn"` | Waline 后端服务地址 |
| `waline.lang` | `string` | `"zh-CN"` | 评论系统语言 |
| `waline.login` | `"enable" \| "force" \| "disable"` | `"disable"` | 评论登录模式 |
...
| 模式 | 说明 |
|------|------|
| `enable` | 允许访客匿名评论和第三方 OAuth 登录，兼容性最佳 |
...
| 配置项 | 类型 | 默认值 | 说明 |
|--------|------|--------|------|
| `giscus.repo` | `string` | `"CuteLeaf/Firefly"` | GitHub 仓库名（格式：username/repo） |
| `giscus.repoId` | `string` | `"R_kgD2gfdFGd"` | 仓库 ID |
| `giscus.category` | `string` | `"General"` | Discussions 分类名 |
| `giscus.categoryId` | `string` | `"DIC_kwDOKy9HOc4CegmW"` | 分类 ID |
| `giscus.mapping` | `string` | `"title"` | 页面与 discussion 的映射方式 |
| `giscus.strict` | `string` | `"0"` | 是否启用严格模式 |
| `giscus.reactionsEnabled` | `string` | `"1"` | 是否启用反应功能 |
| `giscus.emitMetadata` | `string` | `"1"` | 是否发送讨论元数据 |
| `giscus.inputPosition` | `string` | `"top"` | 评论输入框位置 |
| `giscus.lang` | `string` | `"zh-CN"` | 评论系统语言 |
| `giscus.loading` | `string` | `"lazy"` | 加载方式 |
