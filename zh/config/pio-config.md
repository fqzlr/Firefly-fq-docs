...
配置文件路径：`src/config/pioConfig.ts`
...
| 配置项 | 类型 | 默认值 | 说明 |
|--------|------|--------|------|
| `responsive.hideOnMobile` | `boolean` | `false` | 是否在移动端隐藏 |
...
Live2D 配置导出为 `live2dModelConfig`。
...
export const live2dModelConfig: Live2DModelConfig = {
  ...
  interactive: {
    enabled: true,
    clickMessages: ["你好！我是Miku~", "有什么需要帮助的吗？"],
    messageDisplayTime: 3000,
  },
  responsive: {
    hideOnMobile: false,
